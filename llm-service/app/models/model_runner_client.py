...
class ModelRunnerClient:
    ...
    def __init__(self):
        self.client = None
        self.model_name = "omar80/shato-gemma-270m-finetuned:latest"
        ...
        host_endpoint = os.getenv(
            "MODEL_RUNNER_FALLBACK_URL", "http://localhost:11434/v1"
        )
        ...
        if os.getenv("MODEL_RUNNER_URL"):
            ...
            logger.info(
                f"[LLM-SERVICE] Using explicit MODEL_RUNNER_URL: {self.base_url}"
            )
        elif os.path.exists("/.dockerenv"):
            self.base_url = container_endpoint
            logger.info(
                f"[LLM-SERVICE] Detected container environment, using: {self.base_url}"
            )
        else:
            self.base_url = host_endpoint
            logger.info(
                f"[LLM-SERVICE] Detected host environment, using: {self.base_url}"
            )
    ...
    def initialize_client(self):
        ...
        try:
            ...
            logger.info(
                "[LLM-SERVICE-SUCCESS] Model Runner client initialized successfully"
            )

        except Exception as e:
            logger.error(
                f"[LLM-SERVICE-ERROR] Failed to initialize Model Runner client: {str(e)}"
            )
            raise

    async def generate_response(
        self, user_input: str, system_prompt: str = "", retry_context: str = None
    ) -> str:
        """
        ...
        if self.client is None:
            raise RuntimeError(
                "Model Runner client not initialized. Call initialize_client() first."
            )
        ...
        try:
            ...
            logger.info(
                f"[LLM-SERVICE] Sending request to fine-tuned SHATO model: {user_input}"
            )
            logger.info(
                f"[LLM-SERVICE] System prompt length: {len(system_prompt)} chars"
            )
            if retry_context:
            ...
            logger.error(
                f"[LLM-SERVICE-ERROR] Model Runner generation failed: {str(e)}"
            )
            return json.dumps(
                {
    ...
    async def health_check(self) -> dict:
        ...
        try:
            ...
            return {"model_runner_healthy": False, "error": str(e)}
